	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#captchaImage").click(function(){//点击更换验证码
                var src = $("#captchaImage").attr("src")+"?nocache="+new Date().getTime();
                $("#captchaImage").attr("src",src);
			});
			
			//  form 表单提交
			$("#loginForm").bind("submit",function(){
                var isRem = $("#isRememberUsername").prop("checked");
                console.log(isRem);
                if(isRem==true){
			...
			<form id="loginForm" action="/manager/login" method="post" >
				...
				<table>
					...
					<tr>
						...
						<td>
							<label>
								<input type="checkbox" name="isRem" id="isRememberUsername" value="" /> 记住用户名
							</label>
						</td>
					</tr>
					<tr>
